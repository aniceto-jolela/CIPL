<?php	
    ...
    $html.="<div id='tabela_top2'>
            ...
            ";
    $html.="<p id='data'>AUMENTO DE SALÁRIO REFERENTE AO MÊS DE ".$data." DE ".$ano." </p>";
    ...
    $html.="<tr>
            ...
            $aumento = Aumento::findBySql(con(), "SELECT * FROM aumento ORDER BY funcionario_id");
            $ver = Cargo::findBySql(con(), "SELECT * FROM cargo ORDER BY nome");
            $id = 0; $saldo = 0;$aument=0;$slAntigo=0;$beneficioTot=0;
        foreach ($aumento as $i):
            
            
            $funcionario = Funcionario::findById(con(), $i->getFuncionarioId());   
            $cargo = Cargo::findById(con(), $funcionario->getCargoId());$pega=0;
            $slAntigo = $funcionario->getSalarioBase()-$i->getAumento();
            foreach ($ver as $c) {
            ...
            $aument += $i->getAumento(); 
            $beneficioTot += $i->getAumento()+$funcionario->getSalarioBase();
        endforeach;
        $html.="<tr>";
            $html.="<td colspan='2'></td>";
            $html.="<td><b>TOTAL</b></td>";
            $html.="<td><b>".number_format($saldo,2,",",".")." Kz</b></td>";
            $html.="<td><b>".number_format($aument,2,",",".")." Kz</b></td>";
