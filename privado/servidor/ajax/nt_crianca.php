...
<script>
    ...
    function visto(i,id){
       ...
       if(v != "fa fa-eye-slash"){
           ...
           $.ajax({
               type:"GET",
               url:"../servidor/e_notificacao_crianca.php?codigo=0&id="+id,
               dataType:"json",
               data:dados,
               success:function(i){
                   if(i.numero === '0'){mVisto();nFCriancaAlert();}
               },
               error:function(){
    ...
    function vTodos(){
        ...
        if(pega==1){
            /*--------------------------- Area do AJAX ----------------------------------*/
                ...
                $.ajax({
                    type:"POST",
                    url:"../servidor/e_notificacao_crianca.php",
                    dataType:"json",
                    data:dados,
                    success:function(i){
                        if(i.numero === '1'){mVisto();nFCriancaAlert();}
                    },
                    error:function(){
                        alert("Surgiu um erro");
                    }
    ...
    /*=============================== Mostrar Tabelas ==================================================*/
        ...
        function mVisto(){
            var dados = $("#fNF").serialize();
            $.post("../servidor/v_notificacao_crianca.php",dados,function(retorno){
                $("#nf").html(retorno);
            });
        }
         function mVistoIndex(i,id){
            var dados = $("#fNF").serialize();
            $.post("../servidor/e_notificacao_crianca.php?cod="+id,dados,function(retorno){
                if(retorno == "sucesso"){
    ...
    /*=============================== Mostrar o alert (número) da notificação ===============================*/
       nFCriancaAlert();
        function nFCriancaAlert(){
            var dados = $("#fNF").serialize();
            $.post("../servidor/v_nfalert_crianca.php",dados,function(retorno){
    ...
    function pesquisa(i){
        var dados = $("#fNF").serialize();
        if(i!=""){
            $.post("../servidor/vp_notificacao_crianca.php?pn="+i,dados,function(retorno){
                $("#nf").html(retorno);
            });
        }else{
             $.post("../servidor/v_notificacao_crianca.php",dados,function(retorno2){
                $("#nf").html(retorno2);
            });
        }
    }
